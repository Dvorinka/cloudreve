...
- [x] #199 (upstream #3584) — markdown editor lag: `onChange`→`changedValue`→`markdown` prop feedback loop re-parsed the whole document per keystroke; `markdown` now bound to `initialValue` only and the plugins array is memoized — no library swap needed
